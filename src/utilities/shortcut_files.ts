...
export function createFile(file: fileStore){
    ...
    editFile.addEventListener("click", (event)=>{
        event.preventDefault();
        event.stopPropagation();
        if (document.getElementById("edit_file_menu")) return //prevent spam
        const EditFileMenuContent = document.createElement("div");
        EditFileMenuContent.id = "edit_shortcut_menu"
        document.body.appendChild(EditFileMenuContent)

        EditFileMenuContent.innerHTML = `
            <div id="edit_file_menu">
                <div style="padding: 10px;">
                    <strong>Edit File</strong>
                </div>
            <form>
                <label for="edit_file_name">File Name</label>
                <input type="text" id="editfilename" name="Edit_File_Name"><br><br>
            </form>
            <button id=save_file_edit>Save</button>
            <button id=cancel_file_edit>Cancel</button>
            </div>
        </div>
        `;

        const editFileNameBar = EditFileMenuContent.querySelector<HTMLInputElement>("#editfilename")!;
        editFileNameBar.value = file.fileName;

        const SaveFileEditButton = EditFileMenuContent.querySelector<HTMLButtonElement>("#save_file_edit")!;
        SaveFileEditButton.addEventListener("click", (e)=>{
            ...
            loadShortcuts();
            loadFiles();
            EditFileMenuContent.remove();
        });

        const CancelEditButton = EditFileMenuContent.querySelector<HTMLButtonElement>("#cancel_file_edit")!;
